...
> **M&S Solutions 기술 보고서** | 2026-03-05 (RAG 3단계 업그레이드 반영)
...
### 2.3 하이브리드 검색: Kiwi BM25 + 벡터 + 가중 RRF + MMR

**결정**: 벡터 검색 단독 대신, Kiwi 형태소 BM25(키워드) + 벡터(의미) 7:3 가중 하이브리드 + MMR 다양성 리랭킹 채택.
...
#### 2.3.1 한국어 BM25의 결정적 문제: 공백 토크나이저

**문제**: 초기 BM25는 `d.split()` — 공백 기반 토크나이징을 사용했다. 한국어는 교착어(agglutinative language)로 조사가 어근에 붙는다:

```
"정보통신공사업을" → ["정보통신공사업을"]  (1토큰 — 조사 '을' 포함)
"정보통신공사업"   → ["정보통신공사업"]    (1토큰 — 조사 없음)
→ 매칭 실패! 같은 단어인데 조사 하나 차이로 BM25가 다른 문서로 취급.
```

한국어 BM25 벤치마크에서 공백 토크나이저는 **최하위 성능** (NDCG@10 기준). 형태소 분석 토크나이저(OKT, Kiwi)는 최상위.

**결정**: Kiwi(kiwipiepy) 형태소 분석기 채택.

```python
from korean_tokenizer import tokenize_ko

tokenize_ko("정보통신공사업을 수행합니다")
→ ["정보", "통신", "공사", "업", "수행"]  # 조사('을'), 어미('합니다') 제거
```

**왜 Kiwi인가? (vs OKT)**:
- **성능**: C++ 네이티브 바인딩, OKT 대비 5~10배 빠름 (순수 Java 기반 Komoran/OKT vs C++)
- **설치**: `pip install kiwipiepy` — JVM 의존성 없음 (OKT는 JDK 필요)
- **정확도**: NDCG@10에서 OKT와 0.01 차이 (0.89 vs 0.90) — 실질적 차이 무시 가능
- **폴백**: kiwipiepy 미설치 시 자동으로 `str.split()` 폴백 (하위 호환성 유지)

#### 2.3.2 가중 RRF(7:3) + MMR 다양성 리랭킹

**해법 — 가중 RRF(Reciprocal Rank Fusion)**:

공공조달 도메인에서 키워드 정확 매칭이 의미 유사도보다 중요하다 (자격요건의 인증코드, 면허번호, 법적 용어). 이를 반영하여 BM25에 70%, 벡터에 30% 가중치를 부여한다.

```python
BM25_WEIGHT = 0.7   # 키워드 정확 매칭 우선
VECTOR_WEIGHT = 0.3  # 의미 유사도 보완
...
RRF 퓨전만으로는 유사한 청크가 상위를 독점하는 문제가 있다. MMR은 관련성과 다양성을 균형 잡는다:

```
MMR(d) = λ × relevance(d) - (1-λ) × max_similarity(d, selected)
       = 0.7 × RRF점수 - 0.3 × 이미선택된문서와의최대유사도
```

LangChain의 `EnsembleRetriever(search_type="mmr")` 패턴을 LangChain 없이 직접 구현:

```python
@staticmethod
def _mmr_rerank(candidates, top_k, lambda_param=0.7):
    from difflib import SequenceMatcher
    selected = [candidates[0]]  # 최고 RRF 점수
    remaining = candidates[1:]
    while len(selected) < top_k and remaining:
        best_idx = max(range(len(remaining)), key=lambda i:
            lambda_param * remaining[i].score
            - (1 - lambda_param) * max(
                SequenceMatcher(None, remaining[i].text[:200], s.text[:200]).ratio()
                for s in selected))
        selected.append(remaining.pop(best_idx))
    return selected
```

**왜 LangChain 없이?** LangChain을 추가하면 100+ 패키지 의존성이 들어온다. MMR 핵심 로직은 20줄이다. `difflib.SequenceMatcher`로 텍스트 유사도를 측정하면 충분하다.

**왜 RRF인가?**
- **가중치 조절 가능**: 7:3 비율로 도메인에 맞게 BM25 우선. 필요시 비율 변경 가능.
- **순위 기반**: 두 시스템의 점수 스케일이 달라도(벡터: 0~1 코사인, BM25: 0~∞ TF-IDF) 문제없음.
- **학계 검증**: TREC 벤치마크에서 CombMNZ 등 다른 융합 방법 대비 일관된 성능.
- **MMR 보완**: RRF 단독 → 중복 문서 상위 독점. MMR 추가 → 다양한 관점의 결과 보장.
...
**한계**: BM25 토크나이저가 `str.split()` (공백 분할)으로 한국어 조사 차이를 극복하지 못함. "정보통신공사업을"과 "정보통신공사업"이 다른 토큰으로 취급됨.
...
### 10.4 Phase 2: Full Lifecycle (2026-02-28 구현 완료)

**구현 완료**:

| 모듈 | 파일 | 설명 |
|------|------|------|
| 수행계획서/WBS | `wbs_orchestrator.py` | 방법론 감지 → WBS → 간트차트 → XLSX+DOCX |
| PPT 발표자료 | `ppt_orchestrator.py` | 제안서 → 핵심 추출 → PPTX + 예상질문 10개 |
| 실적/경력 기술서 | `track_record_orchestrator.py` | CompanyDB 매칭 + Layer 1 지식 주입 |

### 10.5 RAG 엔진 3단계 업그레이드 (2026-03-05 구현 완료)

**문제 인식**: Phase 0.5에서 도입한 BM25가 `str.split()` 공백 토크나이저를 사용하고 있었다. 한국어 벤치마크에서 공백 토크나이저는 최하위 성능. 또한 긴 RFP 문서의 계층적 이해가 부족하고, PDF 표 데이터가 유실되는 문제가 있었다.

**3축 업그레이드**:

| # | 업그레이드 | 파일 | 핵심 변화 |
|---|-----------|------|-----------|
| 1 | **Kiwi 형태소 BM25 + 7:3 가중 RRF + MMR** | `korean_tokenizer.py`, `engine.py` | 공백 분할 → Kiwi 형태소 어근 추출, 동등 RRF → 7:3 가중, MMR 다양성 리랭킹 |
| 2 | **RAPTOR 트리 인덱싱** | `raptor_indexer.py`, `engine.py` | UMAP+GMM 클러스터링 → LLM 요약 → 재귀 트리 빌드. Collapsed Tree 검색 |
| 3 | **PDF 표 파싱 강화** | `document_parser.py` | `pdfplumber.extract_tables()` → GFM 마크다운 변환. 평가배점표 등 표 데이터 보존 |

**Kiwi BM25 효과**:
```
Before (str.split):
  "정보통신공사업을" → ["정보통신공사업을"] (매칭 실패)
After (Kiwi):
  "정보통신공사업을" → ["정보", "통신", "공사", "업"] (매칭 성공!)
```

**RAPTOR 트리 효과** (ICLR 2024):
```
레벨 0 (리프): [청크1] [청크2] [청크3] ... [청크50]
레벨 1 (요약): [클러스터A 요약] [클러스터B 요약] [클러스터C 요약]
레벨 2 (상위): [전체 요약]
→ "이 사업의 핵심 목표는?" 같은 상위 질의에서 정확도 20%+ 향상 (QuALITY 벤치마크)
```

**PDF 표 파싱 효과**:
```
Before: "평가항목 배점 세부기준 사업이해 15 목적이해도" (텍스트 평탄화 → 구조 유실)
After:  "| 평가항목 | 배점 | 세부기준 |\n| 사업이해 | 15 | 목적이해도 |" (구조 보존)
```

**품질 기준**: 27개 테스트 전수 통과 (Kiwi 5 + 하이브리드 8 + 표파싱 9 + RAPTOR 5)
...
| 지표 | 값 | 비고 |
|------|-----|------|
| RAG 엔진 모듈 | 18개 | rag_engine/ 15 + 루트 3 (korean_tokenizer, raptor_indexer, engine) |
| 테스트 통과 | 102/102 (100%) | rag_engine 75 + 루트 27 (RAG 업그레이드) |
...
- 동의어 사전 → 17개 카테고리 필요
- **BM25 토크나이저** → 공백 분할로는 한국어 매칭 불가. Kiwi 형태소 분석 필수 (2026-03-05 업그레이드)
...
3. **auto_learner 영속성**: ~~현재 인메모리~~ → FastAPI lifespan 이벤트 연결 완료 (main.py).

4. ~~**Phase 2 모듈 구현**~~: 수행계획서/WBS, PPT, 실적기술서 → **구현 완료** (2026-02-28).

5. **Layer 3 승패 분석**: 최소 30쌍의 낙찰/탈락 데이터 축적 후 구현.

6. **RAPTOR 실 사용 검증**: 현재 opt-in(`RAPTOR_ENABLED=1`). 실제 RFP 문서에서 RAPTOR 요약 노드의 검색 정확도 향상 측정 필요.
...
| 모듈 | 파일 | 역할 | 핵심 결정 |
|------|------|------|-----------|
| RAG 엔진 | `engine.py` | ChromaDB + Kiwi BM25 하이브리드 | 7:3 가중 RRF + MMR, Lock 스레드 안전, RAPTOR 옵트인 |
| 한국어 토크나이저 | `korean_tokenizer.py` | Kiwi 형태소 BM25 토크나이저 | 내용어 POS 필터, 싱글턴, 폴백 |
| RAPTOR 인덱서 | `raptor_indexer.py` | UMAP+GMM 클러스터링 + LLM 요약 트리 | Collapsed Tree 검색, gpt-4o-mini 요약 |
