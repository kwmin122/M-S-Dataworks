...
def test_no_api_key_unknown_query_defaults_to_allow() -> None:
    """no api_key + prefilter 미결정 질의는 기본 ALLOW"""
    decision = route_user_query(
...
def test_no_api_key_budget_query_allow_but_context_policy_can_block() -> None:
    """no api_key 기본 ALLOW라도 문맥 없으면 최종 BLOCK_INSUFFICIENT_CONTEXT"""
    decision = route_user_query(
        message="사업 예산이 얼마야?",
        api_key="",
    )
